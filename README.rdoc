...
The code for the Ruby wedge serves as a good example of writing a wedge.
  ...
  require 'rbconfig'
  require 'rbconfig/loader'

  class RbConfig::RubyLoader < RbConfig::Loader
...
The `Loader` base class is just a convenience class. Load wedges can be easily
...
There is also a `RbConfig::LoadWedge::Helper` mixin which provides a few methods
that can be generally useful to any load wedge.
