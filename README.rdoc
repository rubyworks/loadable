...
Under the hood, this simply appends the instance of the $LOADERS globabl variable.
...
The `Loadable` mixin is just a convenience module that makes writting loaders
...
operation. The $LOADERS global varaible is iterated over in order.
...
OriginalLoader which should alwasy be the last loader in the $LOADERS list.
...
The Gem wedge also supports version constratins, so you do not need use 
`gem()` method.
...
This is especailly useful for development. Vendored projects can be added
in two ways, by regisitering an instance of VendorLoader, per the above examples,
