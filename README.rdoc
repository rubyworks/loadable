...
The Load gem provides an easy to use interface for adding custom
...
load interfence between ruby and gems packages libraries.
...
Installing via RubyGems follows the usual pattern. The gem is simply
called `load`.

  $ gem install load


=== Building Load Wedges
...
The `Load::Wedge` base class is hust a conveience class. One could write a load
...
Load is distributed under the terms of the BSD 2-Cluase License.
